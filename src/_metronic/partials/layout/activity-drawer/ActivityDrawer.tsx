...
import { currentFile } from '../../../layout/file_manage'
...
const ActivityDrawer: FC = () => {
  console.log("In activity Drawer", { currentFile });
  // let file_id = 0;
  // console.log("file_id ", file_id);

  // Add state to force re-render when currentFile changes
  const [rerenderKey, setRerenderKey] = useState(0);

  useEffect(() => {
    // Update the rerenderKey when currentFile changes
    ...
    >
      <div className='card shadow-none rounded-0' style={{width: '100%', maxWidth: '1115px'}}>
        <div className='card-header' id='kt_activities_header'>
          <h3 className='card-title fw-bolder text-dark my-0'>Activity Logs {currentFile}</h3>
          {currentFile}
